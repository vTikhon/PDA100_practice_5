import currency.currency_checker
...
# • Дать пользователю выбор любой валюты, с которой работает ЦБ РФ
# • Подумать, как хранить перечень доступных валют
# • Давать возможность пользователю как менять рубли на доступную валюту, так и менять любую доступную на рубли
...
def asker_for_changing_cash(list_):
    ...
    while flag:
        ...
        if answer_.strip().lower() == 'нет':
            ...
            while flag:
                currency_name_ = input('С какой валютой вы хотите совершать операции: ')
                if currency.currency_checker.check_currency_name(currency_name_, list_):
                    while flag:
                        variant_ = input(
                            f'Вы хотите поменять: 1. рубли на {currency_name_} или 2. {currency_name_} на рубли ? 0. выход из вопроса: ')
                        if variant_.strip() == '0':
                    ...
                    print(f'{currency_name_} нет в списке ЦБ')
                    flag = False
        else:
